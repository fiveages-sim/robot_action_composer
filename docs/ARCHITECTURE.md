...
| `movej_return.py` | 连接时缓存关节角、MoveJ 回初始姿态 |
...
- **`context.QueueRuntimeContext`**：一次队列运行的会话（**`interface`**、**`robot_cfg`**、**`sim_time`**、合并后的 **`task_cfg`**（`QueueSingleArmSlice`）、双臂 home、**`drawer` / `drawer_geometry`**、**`handover_sync`**、**`carry_task_cfg` / `carry_object_center`** 等）。
